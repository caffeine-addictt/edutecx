...
def user_lookup_error_loader(jwtHeader, jwtPayload):
  reloginResponse = make_response(redirect(request.path, HTTPStatusCode.SEE_OTHER))
  ...
  if request.path.startswith('/api/'):
    ...
    try:
      ...
      successfulRefresh = make_response(
        redirect(request.path, code=HTTPStatusCode.FOUND), HTTPStatusCode.FOUND
      )
      ...
      app.logger.error(
        f'Failed to Auto-Refresh expired access token with refresh token: {e}'
      )
  ...
  if request.method == 'POST' and form.validate_on_submit():
    ...
    if response.status_code != HTTPStatusCode.OK:
      ...
      successfulLogin = make_response(
        redirect(callbackURI, code=HTTPStatusCode.SEE_OTHER), HTTPStatusCode.SEE_OTHER
      )
...
def logout(user: UserModel):
  successfulLogout = make_response(
    redirect('/', code=HTTPStatusCode.SEE_OTHER), HTTPStatusCode.SEE_OTHER
  )
  ...
  try:
    ...
    if decoded_access.get('jti'):
      db.session.add(JWTBlocklistModel(str(decoded_access.get('jti')), 'access'))
    if decoded_refresh.get('jti'):
      db.session.add(JWTBlocklistModel(str(decoded_refresh.get('jti')), 'refresh'))
    db.session.commit()
...
def verify(user: UserModel, token: str | None = None):
  ...
  if token:
    if (
        ...
        or (user.token.token != token)
        ):
      flash('Invalid token', 'danger')
      newCallbackURI = parse.quote_plus(request.args.get('callbackURI', ''))
      return redirect(
        '/verify?callbackURI=%s' % newCallbackURI, code=HTTPStatusCode.FOUND
