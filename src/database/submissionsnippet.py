...
class SubmissionSnippetModel(db.Model):
  ...
  id: Mapped[str] = mapped_column(
    String, primary_key=True, unique=True, nullable=False, default=lambda: uuid.uuid4()
  )
  ...
  status: Mapped[SnippetUploadStatus] = mapped_column(
    EnumSnippetUploadStatus, nullable=False, default='Uploading'
  )
  student: Mapped['UserModel'] = relationship('UserModel', back_populates='snippets')
  submission: Mapped['SubmissionModel'] = relationship(
    'SubmissionModel', back_populates='snippet'
  )
  ...
  def __init__(
    self, student: 'UserModel', submission: 'SubmissionModel', upload: FileStorage
  ) -> None:
    """
    Submission Snippet Model
