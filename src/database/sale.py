...
from sqlalchemy import Enum, Float, String, Boolean, DateTime, ForeignKey
...
if TYPE_CHECKING:
  ...
  from .association import sale_textbook_association
...
class SaleModel(db.Model):
  ...
  discount_id: Mapped[Optional[str]] = mapped_column(
    ForeignKey('discount_table.id'), nullable=True
  )
  ...
  used_discount: Mapped[Optional['DiscountModel']] = relationship(
    'DiscountModel', back_populates='used_by'
  )
  data: Mapped[List['sale_textbook_association']] = relationship(
    'sale_textbook_association', back_populates='sale'
  )
  ...
  def __init__(
    ...
    *,
    ...
    if saleType == 'OneTime':
      from .association import sale_textbook_association as sta
  ...
  def textbooks(self) -> Dict[str, SaleInfo]:
    ...
    return {
      association.textbook.id: SaleInfo(association.cost, association.textbook)
      for association in self.data
    }
