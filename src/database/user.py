...
if TYPE_CHECKING:
  ...
  from .association import user_textbook_association
...
EnumSubscriptionStatus = Enum(
  'Active', 'Inactive', 'Cancelled', name='SubscriptionStatus'
)
...
class ClassroomMember:
  ...
  def __init__(
    self, user: 'UserModel', classroom: 'ClassroomModel', role: ClassroomMemberType
  ) -> None:
...
class UserModel(db.Model):
  ...
  status: Mapped[UserStatus] = mapped_column(
    EnumUserStatus, nullable=False, default='Active'
  )
  privilege: Mapped[PrivilegeType] = mapped_column(EnumPrivilegeType, nullable=False)
  membership: Mapped[MembershipType] = mapped_column(
    EnumMembershipType, nullable=False, default='Free'
  )
  password_hash: Mapped[str] = mapped_column(String, nullable=False)
  email_verified: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)

  # Misc
  token: Mapped[Optional['TokenModel']] = relationship(
    'TokenModel', back_populates='user'
  )
  profile_image: Mapped[Optional['ImageModel']] = relationship(
    'ImageModel', back_populates='user'
  )
  ...
  submissions: Mapped[List['SubmissionModel']] = relationship(
    'SubmissionModel', back_populates='student'
  )
  snippets: Mapped[List['SubmissionSnippetModel']] = relationship(
    'SubmissionSnippetModel', back_populates='student'
  )
  classrooms: Mapped[List['ClassroomModel']] = relationship(
    'ClassroomModel', secondary='classroom_user_association', back_populates='members'
  )
  owned_classrooms: Mapped[List['ClassroomModel']] = relationship(
    'ClassroomModel',
    primaryjoin='UserModel.id == ClassroomModel.owner_id',
  ...
  textbooks: Mapped[List['TextbookModel']] = relationship(
    'TextbookModel', secondary='user_textbook_association', back_populates='bought_by'
  )
  owned_textbooks: Mapped[List['TextbookModel']] = relationship(
    'TextbookModel',
    primaryjoin='UserModel.id == TextbookModel.author_id',
  ...
  subscription_status: Mapped[SubscriptionStatus] = mapped_column(
    EnumSubscriptionStatus, nullable=False, default='Inactive'
  )
  transactions: Mapped[List['SaleModel']] = relationship(
    'SaleModel',
    primaryjoin='and_(SaleModel.user_id == UserModel.id, SaleModel.paid == True)',
    back_populates='user',
  )
  pending_transactions: Mapped[List['SaleModel']] = relationship(
    'SaleModel',
    primaryjoin='and_(SaleModel.user_id == UserModel.id, SaleModel.paid != True)',
  ...
  def delete(self) -> None:
    ...
    for i in self.transactions:
      i.delete()
    for i in self.owned_textbooks:
      i.delete()
    for i in self.owned_classrooms:
      i.delete()
