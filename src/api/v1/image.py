...
def image_get_api(user: UserModel):
  ...
  if not isinstance(image, ImageModel):
    return GenericReply(
      message='Unable to locate image', status=HTTPStatusCode.BAD_REQUEST
  ...
  if (
    ...
    and (not image.classroom.is_member(user))
  ):
    return GenericReply(
  ...
  return ImageGetReply(
    ...
    data=_ImageGetData(uri=image.uri, image_id=image.id),
...
def image_create_api(user: UserModel):
  ...
  if len(list(filter(None, [req.classroom_id, req.textbook_id, req.user_id]))) != 1:
    return GenericReply(
      message='Invalid identifier', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST

  upload = request.files.get('upload')
  if not upload:
    return GenericReply(
      message='No file supplied', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  if req.user_id:
    author = (
      ...
      else UserModel.query.filter(UserModel.id == req.user_id).first()
    )
    ...
    if not isinstance(textbook, TextbookModel):
      return GenericReply(
        message='Invalid textbook', status=HTTPStatusCode.BAD_REQUEST
      ).to_dict(), HTTPStatusCode.BAD_REQUEST
    ...
    return GenericReply(
      message='No parent supplied', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  return ImageCreateReply(
    ...
    status=HTTPStatusCode.OK,
    data=_ImageCreateData(image_id=newImage.id, uri=newImage.uri),
  ).to_dict(), HTTPStatusCode.OK
...
def image_delete_api(user: UserModel):
  ...
  if (user.privilege != 'Admin') and (
    ...
    or (image.classroom and (user.id != image.classroom.owner_id))
    or (image.textbook and (user.id != image.textbook.author_id))
  ):
    return GenericReply(
      message='Unauthorized', status=HTTPStatusCode.UNAUTHORIZED
    ).to_dict(), HTTPStatusCode.UNAUTHORIZED
  ...
  return ImageDeleteReply(
    message='Image deleted successfully', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
