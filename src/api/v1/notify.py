...
from src.utils.api import NotifyMakeRequest, NotifyReply, NotifyGetReply, GenericReply

from flask import flash, request, get_flashed_messages, current_app as app
...
def notify_new(user: UserModel | None):
  ...
  if (not req.message) or (req.message == 'None'):
    return GenericReply(
      message='Message cannot be empty', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST

  flash(req.message, req.category)
  return NotifyReply(
    message='Successfully created new notification', status=HTTPStatusCode.OK
