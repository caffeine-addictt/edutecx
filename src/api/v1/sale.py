...
def sale_list_api(user: UserModel):
  ...
  query = [
    and_(
      dateRange[0] <= SaleModel.created_at,
      SaleModel.created_at <= dateRange[1]
    ),
    or_(
  ...
  return SaleListReply(
    ...
    data = [_SaleGetData(
      ...
      textbook_ids = [ i.split(':')[0] for i in (i.textbook_ids).split(',') ] if i.textbook_ids else [],
      paid = i.paid,
      paid_at = i.paid_at,
