...
def comment_get_api(user: UserModel):
  ...
  if not isinstance(comment, CommentModel):
    return GenericReply(
      message='Could not locate comment', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST

  if (
    (user.privilege != 'Admin')
    and (user.id != comment.author_id)
    and (not comment.submission.assignment.classroom.is_privileged(user))
  ):
    return GenericReply(
...
def comment_create_api(user: UserModel):
  ...
  if (not req.text) or (req.text == 'None'):
    return GenericReply(
      message='Invalid text', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  newComment = CommentModel(author=user, submission=submission, text=req.text)
  newComment.save()

  return CommentCreateReply(
    message='Successfully created comment',
    status=HTTPStatusCode.OK,
    data=_CommentCreateData(comment_id=newComment.id),
  ).to_dict(), HTTPStatusCode.OK
