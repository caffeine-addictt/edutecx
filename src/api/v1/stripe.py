...
from src.utils.api import (
  ...
  StripeSubscriptionReply,
  _StripeSubscriptionData,
  StripeCheckoutRequest,
  StripeCheckoutReply,
  _StripeCheckoutData,
  StripeSubscriptionCancelRequest,
  StripeSubscriptionCancelReply,
  StripeExpireRequest,
  StripeExpireReply,
  StripeStatusRequest,
  StripeStatusReply,
...
def create_stripe_subscription_session_api(user: UserModel):
  ...
  if user.membership != 'Free':
    return GenericReply(
      message='Already subscribed', status=HTTPStatusCode.BAD_REQUEST
  ...
  if discount:
    if discount.expires_at and (discount.expires_at < datetime.utcnow()):
      return GenericReply(
        message='Discount expired', status=HTTPStatusCode.BAD_REQUEST
      ).to_dict(), HTTPStatusCode.BAD_REQUEST

    if discount.limit and (discount.limit <= discount.used):
      return GenericReply(
        message='Discount limit reached', status=HTTPStatusCode.BAD_REQUEST
      ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  return StripeSubscriptionReply(
    ...
    data=_StripeSubscriptionData(
      session_id=session['id'], public_key=app.config.get('STRIPE_PUBLIC_KEY', '')
    ),
...
def create_stripe_checkout_session_api(user: UserModel):
  ...
  if not req.cart:
    return GenericReply(
      message='Cart cannot be empty', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  found: list[TextbookModel] = TextbookModel.query.filter(
    TextbookModel.id.in_(req.cart)
  ).all()
  if len(found) != len(req.cart):
    return GenericReply(
      message='Invalid items in cart', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST

  # Ensure textbook is not already owned
  if any((user in i.bought_by) for i in found):
    return GenericReply(
      message='One or more textbook(s) already owned', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
...
def cancel_stripe_subscription_api(user: UserModel):
  ...
  if (not req.subscription_id) or (req.subscription_id == 'None'):
    return GenericReply(
      message='Invalid subscription id', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  sale = SaleModel.query.filter(
    SaleModel.subscription_id == req.subscription_id
  ).first()
  if not isinstance(sale, SaleModel):
    return GenericReply(
      message='Subscription not found', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  return StripeSubscriptionCancelReply(
    message='Subscription will cancel at the end of period', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
...
def stripe_expiresession_api(user: UserModel):
  ...
  if len(pending) == 0:
    return GenericReply(
      message='Pending transaction not found', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  try:
    ...
    app.logger.error(f'Failed to expire session {pending.session_id}: {e}')
    return GenericReply(
      message='Failed to expire session', status=HTTPStatusCode.INTERNAL_SERVER_ERROR
    ).to_dict(), HTTPStatusCode.INTERNAL_SERVER_ERROR

  pending.delete()

  return StripeExpireReply(
    message='Checkout cancelled', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
...
@limiter.limit(
  '10/second', scope=lambda _: request.host, key_func=util.get_remote_address
)
def stripe_webhook_api():
  ...
  try:
    ...
    return GenericReply(
      message='Invalid payload', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  match event.type:
    case 'checkout.session.completed':  # Triggered when checkout succeeds
      ...
      if not isinstance(sale, SaleModel):
        return GenericReply(
          message='Failed to locate sale', status=HTTPStatusCode.BAD_REQUEST
        ).to_dict(), HTTPStatusCode.BAD_REQUEST
    ...
    case 'invoice.paid':  # Handle recurring charge
      ...
      if isinstance(sale, SaleModel) and sale.paid:
        ...
        SaleModel(
          user=sale.user, saleType='Subscription', total_cost=sale.total_cost
        ).save()
    ...
    case 'customer.subscription.deleted':
      user = UserModel.query.filter(
        UserModel.subscription_id == event.data.object['subscription']
      ).first()
      if not isinstance(user, UserModel):
        return GenericReply(
          message='Unable to locate customer', status=HTTPStatusCode.BAD_REQUEST
        ).to_dict(), HTTPStatusCode.BAD_REQUEST
  ...
  return GenericReply(
    message='Webhook received', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
