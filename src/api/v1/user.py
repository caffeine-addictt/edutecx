...
def user_get_api(user: UserModel):
  ...
  if not isinstance(foundUser, UserModel):
    return GenericReply(
      message='Unable to lcoate user', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST
...
def user_edit_api(user: UserModel):
  ...
  foundUser = (
    user
    if req.user_id and (req.user_id != 'None') and user.id == req.user_id
    else UserModel.query.filter(UserModel.id == req.user_id).first()
  )
  if not isinstance(foundUser, UserModel):
    return GenericReply(
      message='Unable to locate user', status=HTTPStatusCode.BAD_REQUEST
    ).to_dict(), HTTPStatusCode.BAD_REQUEST

  if req.privilege and (req.privilege != 'None') and (user.privilege != 'Admin'):
    return GenericReply(
      message='Only admins can edit privileges', status=HTTPStatusCode.UNAUTHORIZED
    ).to_dict(), HTTPStatusCode.UNAUTHORIZED

  if req.status and (req.status != 'None') and (user.privilege != 'Admin'):
    return GenericReply(
      message='Only admins can edit status', status=HTTPStatusCode.UNAUTHORIZED
    ).to_dict(), HTTPStatusCode.UNAUTHORIZED
  ...
  return UserEditReply(
    message='Successfully edited user', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
...
def user_delete_api(user: UserModel):
  ...
  return UserDeleteReply(
    message='Successfully deleted user', status=HTTPStatusCode.OK
  ).to_dict(), HTTPStatusCode.OK
